...
namespace Modules\CabBooking\Http\Traits;
...
trait RideRequestTrait
{
    ...
    public function verifyRideWalletBalance($rider_id)
    {
        ...
        if ($rider?->wallet?->balance < 0) {
            throw new Exception(__('cabbooking::static.rides.negative_wallet_balance'), 400);
        }
        return true;
    }
    ...
    public function verifyVehicleType($request)
    {
        ...
        if (!in_array($request?->service_id, [$vehicleType?->service_id])) {
            throw new Exception(__('cabbooking::static.rides.service_not_allow_for_vehicle', ['vehicleType' => $vehicleType?->name]), 400);
        }
        $allowed = $vehicleType?->service_categories()->pluck('service_category_id')?->toArray();
        if (!in_array($request?->service_category_id, $allowed ?? [])) {
            throw new Exception(__('cabbooking::static.rides.category_not_allow_for_vehicle', ['vehicleType' => $vehicleType?->name]), 400);
        }
        return true;
    }
        ...
        try {
            ...
            if (count($rideRequest->location_coordinates ?? [])) {
                if (!in_array($rideRequest?->service_category?->type, [ServiceCategoryEnum::RENTAL, ServiceCategoryEnum::SCHEDULE])) {
                    ...
                    if (!count($zones)) {
                        throw new Exception(__('cabbooking::static.rides.ride_requests_not_accepted'), 400);
                    }
                    ...
                    $minWalletBalance = $cabbookingSettings['wallet']['driver_min_wallet_balance'] ?? 0;
                    ...
                    if (!count($drivers)) {
                        throw new Exception(__('cabbooking::static.rides.no_driver_available'), 400);
                    }
    ...
    public function createCabRideRequest($request)
    {
        ...
        try {
            ...
            if ($serviceCategory?->type == ServiceCategoryEnum::SCHEDULE && $request->start_time) {
                $hours = getCabBookingSettings()['ride']['schedule_min_hour_limit'] ?? 0;
            ...
            if ($this->verifyVehicleType($request)) {
                ...
                $total = $charges['total'];
                if ((int) getCabBookingSettings()['activation']['bidding'] && $request->ride_fare >= $total) {
    ...
    public function addRideRequestFireStore($rideRequest, $drivers)
    {
        ...
        $rideRequest['is_bidding'] = (string) $cabbookingSettings['activation']['bidding'];
        $rideRequest['driver_ride_request_accept_time'] = (string) ($cabbookingSettings['ride']['ride_request_time_driver'] ?? '30');
        $rideRequest['driver_amb_rent_ride_req_time'] = (string) ($cabbookingSettings['ride']['rental_ambulance_request_time'] ?? '30');

        switch ($serviceType) {
            case ServicesEnum::CAB:
            case ServicesEnum::FREIGHT:
            case ServicesEnum::PARCEL:
                $this->handleCabFreightParcel($rideRequest, $drivers, $serviceCategoryType, $cabbookingSettings);
                break;
    ...
    private function handleCabFreightParcel($rideRequest, $drivers, $serviceCategoryType, $cabbookingSettings)
    {
        if (in_array($serviceCategoryType, [ServiceCategoryEnum::RIDE, ServiceCategoryEnum::INTERCITY, ServiceCategoryEnum::SCHEDULE, ServiceCategoryEnum::PACKAGE])) {
            $cabbookingSettings['activation']['bidding']
