...
@section('title', __('cabbooking::static.rides.rides'))
@section('content')
    ...
    <div class="row ride-dashboard">
        <div class="default-sorting mt-0">
            <div class="welcome-box">
                <div class="d-flex">
                    <h2>{{ __('cabbooking::static.rides.ride_details') }}</h2>
                </div>
            </div>
        </div>
            ...
            <div class="card">
                <div class="card-header card-no-border">
                    <div class="header-top">
                        <h5 class="m-0">{{ __('cabbooking::static.rides.general_detail') }}</h5>
                    </div>
                </div>
                <div class="card-body pt-0">
                    <ul class="ride-details-list">
                        <li>
                            {{ __('cabbooking::static.rides.ride_id') }}:
                            <span class="bg-light-primary" id="ride-number-span">#{{ $ride->ride_number }}</span>
                        </li>
                        @if ($ride->start_time)
                            <li>{{ __('cabbooking::static.rides.start_date_time') }} :
                        ...
                        @if (in_array($ride?->service_category?->slug, [ServiceCategoryEnum::PACKAGE, ServiceCategoryEnum::RENTAL]))
                            <li>{{ __('cabbooking::static.rides.end_date_time') }} :
                        ...
                        @endif
                        <li>
                            {{ __('cabbooking::static.rides.ride_status') }} :
                        ...
                        <li>
                            {{ __('cabbooking::static.rides.payment_status') }} :
                        ...
                        @if (in_array($ride?->service_category?->slug, [ServiceCategoryEnum::RENTAL]))
                            <li>
                                {{ __('cabbooking::static.rides.no_of_days') }} : <span
                        ...
                        @if (in_array($ride->service->slug, [ServicesEnum::AMBULANCE]))
                            <li>
                                {{ __('cabbooking::static.rides.ambulance_name') }} :
                                <span id="ambulance-name-span">{{ $ride->driver?->ambulance?->name }}</span>
                            </li>
                        @endif
                        @if (!in_array($ride?->service_category?->slug, [ServiceCategoryEnum::RENTAL]))
                            <li>
                                {{ __('cabbooking::static.rides.ride_distance') }} : <span
                                    id="distance-span">{{ $ride?->distance }}
                                    {{ $ride?->distance_unit }}</span>
                            </li>
                        @endif
                        <li>
                            {{ __('cabbooking::static.rides.waiting_total_times') }} :
                            <span
                                id="waiting-total-times-span">{{ $ride?->waiting_total_times }}{{ __('cabbooking::static.rides.minutes') }}</span>
                        </li>
                        <li id="payment-method-span">
                            {{ __('cabbooking::static.rides.payment_method') }} :
                            <span>
                    ...
                    @if (
                        $ride?->service_category?->slug != ServiceCategoryEnum::RENTAL &&
                        ...
                        <div class="total-bidding">
                            <div class="left-bidding">
                                ...
                                <div class="bidding-text">
                                    <h4>{{ __('cabbooking::static.rides.total_biddings') }}</h4>
                                    <h3>{{ $ride->bids->count() }}</h3>
                                </div>
                            </div>
                            <button class="btn bg-primary" data-bs-toggle="modal"
                                data-bs-target="#bidding">{{ __('cabbooking::static.rides.biddings') }}</button>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-xxl-12">
                    <div class="card h-auto">
                        <div class="card-header card-no-border">
                            <div class="header-top">
                                <h5 class="m-0">{{ __('cabbooking::static.rides.driver_detail') }}</h5>
                            </div>
                        </div>
                        <div class="card-body pt-0">
                            <div class="personal">
                                    ...
                                    @if ($ride?->driver && $ride->driver_id)
                                        <div class="personal-rating">
                                            ...
                                            <div class="rating" id="driver-rating-stars">
                                                <span id="driver-rating">{{ __('cabbooking::static.riders.rating') }}:
                                                    @php
                                ...
                                <ul class="personal-details-list">
                                    <li>
                                        <span>{{ __('cabbooking::static.rides.email') }}: </span>
                                        <span id="driver-email-value">{{ $ride?->driver?->email ?? '' }}</span>
                                    </li>
                                    <li>
                                        <span>{{ __('cabbooking::static.rides.phone') }}:</span>
                                    ...
                                    @if (!in_array($ride->service->slug, [ServicesEnum::AMBULANCE]))
                                        <li>
                                            <span
                                                id="vehicle-number">{{ __('cabbooking::static.riders.vehicle_num') }}:</span>
                                            <span
                                                id="vehicle-number-value">{{ $ride?->driver?->vehicle_info?->plate_number }}</span>
                                        </li>
                                        @if (!in_array($ride?->service_category?->slug, [ServiceCategoryEnum::RENTAL]))
                                            <li>
                                                <span>{{ __('cabbooking::static.rides.vehicle_type') }}: </span>
                ...
                <div class="col-xxl-12">
                    <div class="card h-auto">
                        <div class="card-header card-no-border">
                            <div class="header-top">
                                <h5 class="m-0">{{ __('cabbooking::static.rides.rider_details') }}</h5>
                            </div>
                        </div>
                        <div class="card-body pt-0">
                            <div class="personal">
                                <div class="information">
                                            ...
                                            @else
                                                @isset($ride?->rider['name'])
                                                    <div class="initial-letter" id="rider-initial-container">
                                                        <span
                                                            id="rider-initial">{{ strtoupper($ride?->rider['name'][0] ?? 'G') }}</span>
                                                    </div>
                                                @endisset
                                            @endif
                                        </div>
                                    ...
                                    <div class="personal-rating">
                                        ...
                                        <div class="rating" id="rider-rating-stars">
                                            <span>{{ __('cabbooking::static.rides.rating') }}:
                                                @php
                                                    $averageRating = 0;
                                                    if (
                                ...
                                <ul class="personal-details-list">
                                    ...
                                    <li>
                                        <span>{{ __('cabbooking::static.rides.contact_number') }}:</span>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-header card-no-border">
                            <div class="header-top">
                                <h5 class="m-0">{{ __('cabbooking::static.rides.price_details') }}</h5>
                                <div class="d-flex">
                                    @if ($ride->invoice_id)
                                        <a href="{{ route('ride.rider.invoice', $ride->invoice_id) }}"
                                            class="btn btn-primary">
                                            <i class="ri-download-line"></i>
                                            {{ __('cabbooking::static.rides.rider_invoice') }}
                                        </a>
                                    @endif
                                    @if ($ride->invoice_id)
                                        <a href="{{ route('ride.driver.invoice', $ride->invoice_id) }}"
                                            ...
                                            <i class="ri-download-line"></i>
                                            {{ __('cabbooking::static.rides.driver_invoice') }}
                                        </a>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="card-body pt-0">
                            ...
                            <ul class="price-details-list">
                                ...
                                @if ($ride->additional_minute_charge > 0)
                                    <li>
                                        {{ __('cabbooking::static.rides.additional_minute_charge') }} :
                                        <span
                                ...
                                @if ($ride->additional_weight_charge > 0)
                                    <li>
                                        {{ __('cabbooking::static.rides.additional_weight_charge') }} :
                                        <span
                                ...
                                @if ($ride->total_extra_charge > 0)
                                    <li>
                                        {{ __('cabbooking::static.rides.extra_charge') }} :
                                        <span
                                ...
                                @if ($isRental)
                                    <li>
                                        {{ __('cabbooking::static.rides.vehicle_charge') }} :
                                        <span id="vehicle-charge-span">
                                            ...
                                            {{ __('cabbooking::static.rides.days') }})
                                        </span>
                                    </li>
                                    <li>
                                        {{ __('cabbooking::static.rides.driver_charge') }} :
                                ...
                                @if ($ride->waiting_charges > 0)
                                    <li>
                                        {{ __('cabbooking::static.rides.waiting_charges') }} :
                                        <span
                                ...
                                @if ($ride->bid_extra_amount > 0)
                                    <li>
                                        {{ __('cabbooking::static.rides.bid_extra_amount') }} :
                                        <span
                                ...
                                @if ($ride->driver_tips > 0)
                                    <li>
                                        {{ __('cabbooking::static.invoice.driver_tips') }} :
                                        <span
                                ...
                                <li class="success-text">
                                    {{ __('cabbooking::static.rides.subtotal') }} :
                                    <span
                                ...
                                @if ($ride?->coupon_total_discount)
                                    <li class="danger-text">
                                        {{ __('cabbooking::static.rides.coupon_discount') }}
                                        @if ($ride?->coupon?->code)
                                            (#{{ $ride?->coupon?->code }})
                                        @endif :
                                        <span
                                ...
                                <li>
                                    {{ __('cabbooking::static.rides.platform_fee') }} :
                                    <span
                                        id="platform-fee-span">{{ $cs . number_format($ride?->platform_fees, 2) }}</span>
                                </li>

                                <li>
                                    {{ __('cabbooking::static.rides.tax') }} :
                                    <span id="tax-span">{{ $cs . number_format(round($ride?->tax, 2), 2) }}</span>
                                </li>

                                <li>
                                    {{ __('cabbooking::static.rides.admin_commission') }} :
                                    <span
                                        id="admin-commission-span">{{ $cs . number_format($ride?->commission, 2) }}</span>
                                </li>

                                @if ($ride->processing_fee > 0)
                                    <li>
                                        {{ __('cabbooking::static.rides.processing_fee') }} :
                                        <span
                                ...
                                <li class="total-box">
                                    {{ __('cabbooking::static.rides.total') }} :
                            ...
                            <ul class="comment-box-list">
                                @if ($ride?->comment)
                                    <li>
                                        <h4>{{ __('cabbooking::static.rides.comments') }}</h4>
                                ...
                                <li>
                                    <ul class="price-details-list">
                                        ...
                                        @if (in_array($ride?->ride_status?->slug, [RideStatusEnum::CANCELLED]))
                                            <li>
                                                <h4>{{ __('cabbooking::static.rides.cancellation_reason') }}</h4>
                                                <span id="cancellation-reason">
                                                    {{ $ride->cancellation_reason->title ?? __('cabbooking::static.rides.default_cancel_reason') }}
                                                </span>
                                            </li>
                                        @endif
                                        ...
                                        @if ($totalCharge > 0)
                                            @if ($riderCharge > 0)
                                                <li>
                                                    {{ __('cabbooking::static.rides.rider_cancellation_charge') }} :
                                            ...
                                            @if ($driverCharge > 0)
                                                <li>
                                                    {{ __('cabbooking::static.rides.driver_cancellation_charge') }} :
                                            ...
                                            <li>
                                                <strong>{{ __('cabbooking::static.rides.total_cancellation_charge') }}
                                                    :</strong>
                        ...
                        <div class="parcel-box">
                            ...
                            <ul class="right-list">
                                @if ($ride?->parcel_receiver)
                                    <li><span>{{ __('cabbooking::static.rides.receiver_name') }}:</span>
                                        <span id="receiver-name">{{ $ride?->parcel_receiver['name'] }}</span>
                                    </li>
                                    <li>
                                        <span>{{ __('cabbooking::static.rides.receiver_no') }}:</span>
                                        <span id="receiver-phone">{{ $ride?->parcel_receiver['country_code'] ?? '' }}
                                            {{ $ride?->parcel_receiver['phone'] ?? '' }}</span>
                                    </li>
                                    <li><span>{{ __('cabbooking::static.rides.parcel_otp') }}:</span>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="driver-box">
                            ...
                            <ul class="right-list">
                                <li><span>{{ __('cabbooking::static.rides.vehicle_name') }}:</span>
                                    <span id="rental-vehicle-name">{{ $ride?->rental_vehicle?->name }}</span>
                                </li>
                                @if ($ride?->is_with_driver == 1)
                                    <li><span>{{ __('cabbooking::static.rides.assign_driver_name') }}:</span>
                                        <span id="assigned-driver-name">{{ $ride?->assigned_driver['name'] }}</span>
                                    </li>
                                    <li>
                                        <span>{{ __('cabbooking::static.rides.assign_driver_no') }}:</span>
                                        <span
                                            id="assigned-driver-phone">+{{ $ride?->assigned_driver['country_code'] ?? '' }}
                                            {{ $ride?->assigned_driver['phone'] ?? '' }}</span>
                                    </li>
                                @else
                                    <li><span>{{ __('cabbooking::static.rides.driver_name') }}:</span>
                                        <span id="driver-name-value">{{ $ride?->driver?->name }}</span>
                                    </li>
                                    <li><span>{{ __('cabbooking::static.rides.driver_no') }}:</span>
                                        <span id="driver-phone-value">{{ $ride?->driver?->phone }}</span>
                                    </li>
                                @endif
                                <li><span>{{ __('cabbooking::static.rides.vehicle_registration_no') }}:</span>
                                    <span
        ...
        <div class="col-xxl-7">
            <div class="card maps-view h-auto">
                <div class="card-header card-no-border">
                    <div class="header-top">
                        <div>
                            <h5 class="m-0">{{ __('cabbooking::static.rides.map_view') }}</h5>
                        </div>
                    </div>
                </div>
                <div class="card-body pt-0">
                        ...
                        <div class="accordion-item location-details">
                            <div class="accordion-header contentbox-title">
                                <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                    ...
                                    <h4>{{ __('cabbooking::static.rides.location_details') }}</h4>
                                </button>
                            </div>
            ...
            @if (isset($ride?->riderReview))
                <div class="card h-auto">
                    ...
                    <div class="card-header card-no-border">
                        <div class="header-top">
                            <h5 class="m-0">{{ __('cabbooking::static.rides.rider_reviews') }}</h5>
                        </div>
                    </div>
                        ...
                        @if (isset($ride?->riderReview))
                            <div class="table-responsive h-custom-scrollbar">
                                <table class="table driver-review-table" style="width:100%">
                                    <thead>
                                        <tr>
                                            <th>{{ __('cabbooking::static.rides.driver') }}</th>
                                            <th>{{ __('cabbooking::static.rides.rating') }}</th>
                                            <th>{{ __('cabbooking::static.rides.description') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                                        ...
                                                        @else
                                                            @isset($ride?->riderReview?->driver)
                                                                <div class="initial-letter"
                                                                    id="review-driver-initial-container">
                                                                    <span
                                            ...
                                            <td>
                                                <div class="position-relative">
                                                    ...
                                                    <a href="javascript:void(0);" class="read-more"
                                                        ...
                                                        {{ __('cabbooking::static.rides.read_more') }}
                                                    </a>
                                                </div>
                                            </td>
                                        </tr>
            ...
            @if (isset($ride?->driverReview))
                <div class="card h-auto">
                    <div class="card-header card-no-border">
                        <div class="header-top">
                            <h5 class="m-0">{{ __('cabbooking::static.rides.driver_reviews') }}</h5>
                        </div>
                    </div>
                    <div class="card-body rider-reviews p-0">
                        @if (isset($ride?->driverReview))
                            <div class="table-responsive h-custom-scrollbar">
                                <table class="table driver-review-table" style="width:100%">
                                    <thead>
                                        <tr>
                                            <th>{{ __('cabbooking::static.rides.rider') }}</th>
                                            <th>{{ __('cabbooking::static.rides.rating') }}</th>
                                            <th>{{ __('cabbooking::static.rides.description') }}</th>
                                        </tr>
                                    </thead>
                ...
                <div class="card h-auto p-0">
                    <div class="card-body rider-reviews">
                        @if (isset($ride?->driverReview))
                            <div class="table-responsive h-custom-scrollbar">
                                <table class="table driver-review-table" style="width:100%">
                                    <thead>
                                ...
                                <ul class="booking-number-list">
                                    @foreach ($ride?->ride_status_activities->sortByDesc('created_at') as $activity)
                                        <li>
                                            ...
                                            <div
                                                class="circle {{ \Modules\CabBooking\Models\RideStatus::getActivityClassByStatus($activity->status) }}">
                                            </div>
                                            <div class="booking-number-box">
                                                <div class="left-box">
                                                    ...
                                                    <h6 class="name">
                                                        {{ ucfirst(str_replace('_', ' ', $activity->status)) }}</h6>
                                                    <h6 class="text-pra">
                                                        {{ $activity->ride_status ? $activity->ride_status->description : \Modules\CabBooking\Models\RideStatus::getDescriptionByStatus($activity->status) }}
                                                    </h6>
                                                </div>
                                                <div class="right-box">
                                                    <h6>{{ \Carbon\Carbon::parse($activity->created_at)->format('h:i A') }}
                                                    </h6>
                                                </div>
                                            </div>
                                        </li>
            ...
            <div class="modal-dialog modal-dialog-centered modal-md">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ __('cabbooking::static.rides.biddings') }}</h5>
                                ...
                                @forelse ($ride?->bids as $bid)
                                    <li class="d-flex align-items-center">
                                        ...
                                        <div class="flex-grow-1">
                                            ...
                                            <span>{{ __('cabbooking::static.riders.rating') }}:
                                                @php
                                                    $averageRating = (int) $ride?->driver?->reviews?->avg('rating');
                                                    $totalStars = 5;
                                                @endphp
...
@if ($settings['location']['map_provider'] == 'google_map')
    @includeIf('cabbooking::admin.ride.google')
@elseIf($settings['location']['map_provider'] == 'osm')
    @includeIf('cabbooking::admin.ride.osm')
@endif
            ...
            db.collection('rides').doc(rideId).onSnapshot((doc) => {
                try {
                    ...
                    safeUpdateElement('cancellation-reason', rideData.cancellation_reason ||
                        '{{ __('cabbooking::static.rides.default_cancel_reason') }}');
        ...
        function toggleRiderReview() {
            try {
                ...
                if (fullReview.classList.contains('d-none')) {
                    readMoreLink.innerHTML = "{{ __('cabbooking::static.rides.read_more') }}";
                } else {
                    readMoreLink.innerHTML = "{{ __('cabbooking::static.rides.read_less') }}";
                }
            } catch (error) {
